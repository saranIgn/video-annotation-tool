...
export interface CanvasContextType {
  ...
  setRectPosition: React.Dispatch<React.SetStateAction<{ x: number; y: number }>>;
...
const Canvas = forwardRef<CanvasRef, CanvasProps>(({
            ...
            switch (type) {
              ...
              case "line":
                updatedProperties = {
                  ...
                  x: node.x(),
                  y: node.y(),
                  points: node.points().map((point: number, index: number) =>
  ...
  const isVisible = useCallback(
    (shapeId: string) => {
      const shape = shapes.find((shape: { id: string; }) => shape.id === shapeId);
      return true;
    },
    [currentTime, shapes]
  );
                ...
                case "rectangle":
                  return (
                    <Rectangle
                      ...
                      onDragStart={selectedShapeId ? handleDragStart : undefined}
                      onDragMove={selectedShapeId ? handleDragMove : undefined}
